...
function init() {
    ...
    if (deal13Btn) {
        ...
        debugLog('Added click handler to Deal 13 Tiles button');
    } else {
        debugLog('WARNING: Could not find Deal 13 Tiles button');
    }
    
    if (deal14Btn) {
        deal14Btn.onclick = () => handleDeal(14);
        debugLog('Added click handler to Deal 14 Tiles button');
    } else {
        debugLog('WARNING: Could not find Deal 14 Tiles button');
    }
    ...
    if (sortSuitBtn) {
        sortSuitBtn.addEventListener('click', () => sortHand('suit'));
        debugLog('Added click handler to Sort by Suit button');
    } else {
        console.error('WARNING: Could not find Sort by Suit button');
    }
    
    if (sortNumberBtn) {
        sortNumberBtn.addEventListener('click', () => sortHand('number'));
        debugLog('Added click handler to Sort by Number button');
    } else {
        console.error('WARNING: Could not find Sort by Number button');
    }
...
function displayHand(hand) {
    ...
    hand.forEach((tileCode) => {
        ...
        // Store the original tile code in a data attribute for sorting
        tileEl.setAttribute('data-tile-code', tileCode);
...
// Helper function to get sortNum for a tile
function getSortNum(tileCode) {
    // Extract number from tile code if it's a number tile
    const numMatch = tileCode.match(/^(\d+)/);
    if (numMatch) return parseInt(numMatch[1], 10);
    
    // Handle special tiles according to the specification
    switch(tileCode) {
        // Dragons are 10
        case 'RD': case 'GD': case 'WD': return 10;
        // Flowers are 11
        case 'FL': return 11;
        // Winds in order: N(12), E(13), W(14), S(15)
        case 'N': return 12;
        case 'E': return 13;
        case 'W': return 14;
        case 'S': return 15;
...
// Helper function to get sortCat for a tile
function getSortCat(tileCode) {
    // Check for numbered tiles first
    if (/^\d/.test(tileCode)) {
        const suit = tileCode.slice(-1);
        if (suit === 'B') return 1;    // Bams
        if (suit === 'C') return 2;    // Cracks
        if (suit === 'D') return 3;    // Dots
    }
    
    // Handle special tiles
    switch(tileCode) {
        case 'GD': return 1;   // Green Dragon (Bam)
        case 'RD': return 2;   // Red Dragon (Crack)
        case 'WD': return 3;   // White Dragon (Dot)
        case 'FL': return 4;   // Flowers
        case 'N':              // Winds
        case 'E':
        case 'W':
        case 'S': return 5;    // All winds in same category
        case 'JK': return 6;   // Joker
        default: return 99;    // Unknown
    }
}

// Sort the current hand by suit or number
function sortHand(sortType) {
    try {
        const tilesRow = document.getElementById('tiles-row');
        if (!tilesRow) {
            console.error('WARNING: Could not find tiles row');
            return;
        }
        
        const tiles = Array.from(tilesRow.children);
        if (tiles.length === 0) {
            console.log('No tiles in hand to sort');
            return;
        }
        
        console.log(`=== Sorting hand by ${sortType} ===`);
        
        // Log all tiles before sorting
        console.log('Tiles before sorting:');
        tiles.forEach((tile, index) => {
            const displayText = tile.textContent.trim();
            const tileCode = tile.getAttribute('data-tile-code') || displayText;
            console.log(`  [${index}]: '${displayText}' (code: '${tileCode}', num: ${getSortNum(tileCode)}, cat: ${getSortCat(tileCode)})`);
        });
        
        tiles.sort((a, b) => {
            // Get the original tile codes from data attributes
            const aTileCode = a.getAttribute('data-tile-code') || a.textContent.trim();
            const bTileCode = b.getAttribute('data-tile-code') || b.textContent.trim();
            
            // Get sort attributes for both tiles
            const aNum = getSortNum(aTileCode);
            const bNum = getSortNum(bTileCode);
            const aCat = getSortCat(aTileCode);
            const bCat = getSortCat(bTileCode);
            
            console.log(`Comparing: '${aTileCode}' (display: '${a.textContent.trim()}', num:${aNum}, cat:${aCat}) vs '${bTileCode}' (display: '${b.textContent.trim()}', num:${bNum}, cat:${bCat})`);
            
            if (sortType === 'suit') {
                // Primary sort: by category (suit)
                if (aCat !== bCat) {
                    console.log(`  Different categories: returning ${aCat - bCat}`);
                    return aCat - bCat;
                }
                // Secondary sort: by number (for tiles in same category)
                console.log(`  Same category, comparing numbers: ${aNum} - ${bNum} = ${aNum - bNum}`);
                return aNum - bNum;
            } else {
                // Primary sort: by number
                if (aNum !== bNum) {
                    console.log(`  Different numbers: returning ${aNum - bNum}`);
                    return aNum - bNum;
                }
                // Secondary sort: by category (for tiles with same number)
                console.log(`  Same number, comparing categories: ${aCat} - ${bCat} = ${aCat - bCat}`);
                return aCat - bCat;
            }
        });
        
        // Clear and re-append the sorted tiles
        while (tilesRow.firstChild) {
            tilesRow.removeChild(tilesRow.firstChild);
        }
        tiles.forEach(tile => tilesRow.appendChild(tile));
        
        console.log('Sorted tiles:', tiles.map(t => t.textContent.trim()));
        updateTileSizes(tiles.length);
    } catch (error) {
        console.error('Error in sortHand:', error);
    }
}
